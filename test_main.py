...
if not os.path.exists(opt.imgs_path):
    print("开始解压")
    unzip_file('data/data22078/faces.zip', './imgs')
    print("解压完成")
    ...
    with paddle.fluid.dygraph.guard(place):
        ...
        for pass_id in range(opt.epoch):
            ...
            for batch_id, (data, labels) in enumerate( tqdm(train_loader) ):
                ...
                if batch_id % opt.g_every==0 and batch_id%opt.d_every==0:
                    t="\tD={:.5f}\t G={:.5f}".format(losses[0][-1],losses[1][-1])
                    print(t)
                if batch_id % 50 == 0:
                    # 每轮的生成结果
            ...
            if pass_id % 2==0:
                order =  order_name+ 1+ pass_id//2 -mm
